...
export class ExtrasSelectionComponent implements OnInit {
  EXTRAS: Observable<string[]> = this.burgen.EXTRAS;
  selectedExtras: any = {};

  constructor(
    private burgen: BurgerGeneratorService,
  ...
  selectExtras(): void {
    let extras = Object.keys(this.selectedExtras).filter(
      (x: string) => this.selectedExtras[x]
    );
